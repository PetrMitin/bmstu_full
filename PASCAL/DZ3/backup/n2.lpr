program n2;
{Объект – кольцо (ювелирное изделие).
Поля: масса, название драгоценного металла, и проба.
Методы: процедура инициализации, процедура вывода информации об объекте на экран, функция определения массы драгоценного металла в изделии по его пробе.

Объект – перстень.
Поля: масса перстня без камня, название драгоценного металла, проба, название камня в перстне, масса камня.
Методы: процедура инициализации, процедура вывода информации об объекте на экран, функция определения массы драгоценного металла в изделии,
функция определения полной массы перстня.}
